...
class EmailController extends Controller
{
    ...
    private function getEmailPreview($body, $length = 100)
    {
        if (empty($body)) {
            return 'No content';
        }
        ...
        try {
            ...
            try {
                if ($message->hasHTMLBody()) {
                    // For HTML emails, get HTML directly without processing
                    $body = $message->getHTMLBody();

                    // Optional: Sanitize dangerous scripts
    ...
    private function sanitizeEmailBody($html)
    {
        ...
        $html = preg_replace('/on\w+\s*=\s*["\'][^"\']*["\']/i', '', $html); // Remove inline event handlers
